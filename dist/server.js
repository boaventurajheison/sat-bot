...
if (process.env.NODE_ENV === "production") {
    ...
    app.use((0, cors_1.default)()); // liberado só no modo dev
}
const PORT = Number(process.env.PORT || 3000);
const API_KEY = process.env.BINANCE_API_KEY || "";
const API_SECRET = process.env.BINANCE_API_SECRET || "";
const USE_TESTNET = (process.env.BINANCE_FUTURES_TESTNET || "false").toLowerCase() === "true";
...
async function syncServerTime() {
    try {
        const resp = await axios_1.default.get(`${BASE}/fapi/v1/time`, { timeout: 5000 });
        const serverTime = resp.data?.serverTime;
        if (typeof serverTime === "number") {
            timeOffset = serverTime - Date.now();
            console.log(`⏱️  Time sync OK — offset: ${timeOffset} ms (server: ${serverTime}, local: ${Date.now()})`);
        }
        else {
...
function signQuery(params = {}) {
    ...
    const qs = new URLSearchParams(Object.entries(qsObj).reduce((acc, [k, v]) => {
        acc[k] = String(v);
        return acc;
    }, {})).toString();
    const signature = crypto_1.default.createHmac("sha256", API_SECRET).update(qs).digest("hex");
...
app.get("/futures/account", async (req, res) => {
    ...
    try {
        ...
        const requestedSymbol = req.query.symbol?.toUpperCase();
        ...
        const openPositions = positions
            ...
            .filter((p) => (requestedSymbol ? p.symbol === requestedSymbol : true))
            .map((p) => ({
            symbol: p.symbol,
 ...
 * }
 */
app.get("/futures/report", async (req, res) => {
    if (!API_KEY || !API_SECRET) {
    ...
    try {
        ...
        const percentRelative = initialBalanceEstimate !== 0 ? (totalRealized / initialBalanceEstimate) * 100 : null;
        ...
        return res.json({
            ...
            percentRelativeToInitialBalance: percentRelative === null ? null : Number(percentRelative.toFixed(6)),
